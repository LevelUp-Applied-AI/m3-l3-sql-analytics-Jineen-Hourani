...
WITH DeptAvg AS (
    ...
    SELECT d.name as department_name, AVG(e.salary) as avg_dept_salary
    FROM employees e
    JOIN departments d ON e.department_id = d.department_id
    GROUP BY d.name
),
CompanyAvg AS (
    ...
    SELECT AVG(salary) as avg_comp_salary FROM employees
)
SELECT 
    department_name, 
    ROUND(avg_dept_salary, 2) as avg_salary,       
    ROUND(avg_comp_salary, 2) as company_avg      
FROM DeptAvg, CompanyAvg 
WHERE avg_dept_salary > avg_comp_salary;
...
-- Note: Use actual employee_ids from your employees table
